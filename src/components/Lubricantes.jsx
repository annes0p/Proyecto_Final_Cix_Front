// src/components/Lubricantes.jsx
  ...
  return (
    <div style={{ display: 'flex', gap: '25px', flexDirection: 'column' }}>
      ...
      <div style={{ backgroundColor: '#222733', padding: '25px', borderRadius: '12px', border: '1px solid #2d3548' }}>
        <h3 style={{ margin: '0 0 20px 0', color: '#64dfdf', fontSize: '1.15em', fontWeight: '600' }}>🛢️ Registrar Nuevo Lubricante</h3>
        <form onSubmit={handleGuardar} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', alignItems: 'end' }}>
          ...
          <button type="submit" style={{ padding: '12px', backgroundColor: '#64dfdf', color: '#1a1d24', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', fontSize: '0.9em' }}>
            Guardar Stock
          </button>
        </form>
      </div>
      ...
      {/* TABLA INVENTARIO */}
      <div style={{ backgroundColor: '#222733', padding: '25px', borderRadius: '12px', border: '1px solid #2d3548' }}>
        <h3 style={{ margin: '0 0 20px 0', color: '#64dfdf', fontSize: '1.15em', fontWeight: '600' }}>📦 Almacén de Productos</h3>
        
        {loading ? (
          <p style={{ color: '#94a3b8' }}>Sincronizando inventario...</p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.95em' }}>
            <thead>
              <tr style={{ backgroundColor: '#1a1d24', textAlign: 'left', borderBottom: '1px solid #2d3548' }}>
                <th style={thStyle}>ID</th>
                <th style={thStyle}>Lubricante</th>
                <th style={thStyle}>Marca</th>
                <th style={thStyle}>Viscosidad</th>
            ...
            <tbody>
              {productos.map((prod) => (
                <tr key={prod.id} style={{ borderBottom: '1px solid #2d3548' }}>
                  ...
                  <td style={tdStyle}>
                    <span style={{ 
                      backgroundColor: prod.stock > 10 ? '#1e3a3a' : '#3b2326', 
                      color: prod.stock > 10 ? '#a7f3d0' : '#fca5a5',
                      padding: '4px 10px', borderRadius: '6px', fontSize: '0.8em', fontWeight: '600'
                    }}>
                      {prod.stock} UNID
                    </span>
                  </td>
